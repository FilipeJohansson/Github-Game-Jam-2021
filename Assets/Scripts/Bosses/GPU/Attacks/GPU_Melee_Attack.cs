...
public class GPU_Melee_Attack : IAttack  {
    ...
    IEnumerator DashIntoPlayer(GameObject _owner) {
        GPU_Behaviour gpu_Behaviour = _owner.GetComponent<GPU_Behaviour>();
        
        // Get the actual pos
        Vector3 initialPos = owner.transform.position;

        bool dashing = true;
        gpu_Behaviour.posLocked = true;
        ...
        while (dashing) {
            gpu_Behaviour.isAttacking = true;
        ...
        gpu_Behaviour.isAttacking = false;
    }
